...
| REQ-065 | Momentum & volume confirmation (#224): **`MomentumDivergenceChecker`** and **`VolumeGuidelineChecker`** are pure, static guideline checkers — no DI, callers pass a precomputed RSI/MACD series and the candle set they already have. Momentum compares RSI at the extension wave's end (wave 3, or wave A in a correction) against the final wave's end (wave 5, or wave C) — weaker momentum despite the price extension is the textbook confirming divergence (AC1); missing RSI (no series supplied, or a warm-up-period null at the exact pivot date) is `Indeterminate`, never a guessed fail (AC2). Volume compares wave 3 against wave 5 (impulse) or wave B against waves A/C (correction); a window where every candle reports zero volume (a provider that doesn't supply it, e.g. CoinGecko's free OHLC endpoint) is likewise `Indeterminate` (AC2). Both are always `IsGuideline = true` — a hard-rule Fail never results (AC4). **Computed once per candidate set, never inside `WaveGrammarParser`'s beam-search inner loop** — that loop can evaluate thousands of partitions per parse, so indicator calculation lives in `WaveCandidateGenerator.GenerateParsed`'s post-processing step instead, applying the same `GuidelinePenalty` a parser-internal failed guideline already gets (AC4, unit-tested — `WaveCandidateGeneratorMomentumVolumeTests`). Wired into the two live "count workflow" paths the issue's own motivation names: `WaveVerifier.Verify` (manual Verify, via `WaveVerificationService`'s injected `IIndicatorCalculator`) and `WaveCandidateGenerator.GenerateParsed` (Auto-analyze and the persona panel, via `AutoWaveAnalysisService`/`PersonaPanelAnalysisService`'s injected `IIndicatorCalculator`) — six other `GenerateParsed` call sites (scanner, backtest, top-down, historical-analog, chart-verification, setup-history) are left unwired by default (the new parameters are optional and default to skipping the guidelines), a deliberate, named scope boundary rather than a silent gap. AC5 needed no new plumbing: `WaveValidationPromptBuilder`/`AutoWaveAnalysisPromptBuilder` already iterate every `RuleReport.Rules` row under an "AUTHORITATIVE, do NOT re-derive" header, so the two new guideline rows reach the LLM prompt automatically, with the same "cite, never compute" contract every other rule row already has. Frontend: a `PriceChart` RSI sub-pane (lightweight-charts v5 `addPane`/`paneIndex`, the first multi-pane usage in this codebase), theme-aware, created lazily on toggle-on and its pane reclaimed on toggle-off, wired to a new "RSI" checkbox beside the existing Invalidation/Fib/Targets layer toggles — off by default (AC3). **Bug found and fixed while wiring this**: `AutoWaveRankRunner`'s catch-all previously discarded the real exception type on every transport failure; not directly related to this ticket, but discovered and fixed in the adjacent #184 slice this session, so momentum/volume's own quota-adjacent paths inherit the fix for free | #224 | Fulfilled, with the six secondary `GenerateParsed` call sites named as an explicit, deliberate scope boundary |
| REQ-066 | Drag pivots directly on the chart (#225, the deferred half of #165): grab a placed pivot marker, drag, drop — on release it snaps to the nearest candle extreme via the same `snapToCandle` function placement and nudge already use, and the debounced live-verify fires exactly once, on drop (AC1, AC3). Since lightweight-charts v5 exposes no marker-hit-testing or drag API, pivots are hand hit-tested against the `user` wave-line's own points (the only series carrying both time *and* price per pivot — the marker layer only carries a date) via `chart.timeScale().timeToCoordinate`/`series.priceToCoordinate`, using raw DOM `pointerdown`/`pointermove`/`pointerup` listeners on the chart container (the chart API has no pointer-level subscription, only `subscribeClick`/`subscribeCrosshairMove`). A drag disables the chart's own `handleScroll`/`handleScale` for its duration so it never fights lightweight-charts' native click-to-pan, and is opt-in per instance (`onPivotDragEnd` unset ⇒ the chart behaves exactly as it did before #225, nudge-only). **AC2 (a drag must never also place a new pivot) is solved structurally**: a drag's release fires the chart's own `subscribeClick` callback immediately after (the same physical mouseup), so a one-shot `justDraggedRef` flag set at drag-end suppresses exactly that next click, then clears itself — verified by a test that invokes the registered click handler directly after a simulated drag and asserts `onPointClick` never fires, alongside a sibling test proving an unrelated plain click is unaffected. **AC3 (live leg-readout during drag)** reuses `legMeasurements` unchanged: a new `displayAnnotations` derivation (the committed `annotations` with the in-flight drag substituted in) feeds the leg readout, markers and the `user` wave line, while the live-verify debounce keeps watching only the real `annotations` state — so the readout updates on every pointermove but a re-verify network call fires only once, on drop, exactly matching the existing #165 debounce mechanism with zero changes to it. **AC4 (hands-on verification in the running app, including touch) could not be performed in this sandbox** — no live browser or composed stack was available (same root cause named throughout this session's ADR-064/065/066/069) — component-level tests simulate the pointer gesture via `PointerEvent`/`dispatchEvent` and assert the callback contract, but the feature's actual on-screen feel (snap responsiveness, touch dragging) is unverified until a developer's local run | #225 | Implemented, AC1–AC3 unit-tested; **AC4 explicitly unmet — needs a hands-on pass in a real browser before this ticket can be considered done**, per its own governance line ("hands-on verification required before merge") |
| REQ-067 | Per-symbol workspace drafts + watchlist (#226): switching symbol/timeframe used to unconditionally wipe `annotations` (`handleSymbol`/`handleTimeframe`, unchanged); a new server-side **`WorkspaceDraft`** (one row per `(user, symbol, interval)`, annotations + settings stored as opaque JSON the backend never interprets) is auto-saved 800ms after the last edit and auto-restored on switch, so an in-progress count on one symbol survives a glance at another (AC1) and a reload/different device (AC2, server-side and per-user-scoped like every other track). A draft is distinct from a track-record save: no outcome tracking, silently overwritten by the next auto-save, and deleting one never touches `AnalysisSnapshot` (AC4) — placing zero annotations deletes rather than persists an empty draft, so a mere glance creates nothing. Capped at 50 drafts per user with LRU eviction (oldest `UpdatedAt` first) past the cap. The hardcoded SP500/NASDAQ/BTC/ETH quick-symbol row is replaced by a user-managed **`WatchlistEntry`** list (`GET/POST /api/watchlist`, `DELETE /api/watchlist/{symbol}`), seeded once with those same four legacy symbols on a new user's first read (AC3); each entry reports a best-effort last price (`IQuoteProvider`, already used by depot enrichment — never throws, null when unavailable) and `HasDraft` (any interval). **Bug found by TDD, fixed before merge**: the restore-completion tracker was originally a `useRef`, which does not itself trigger a re-render — when a symbol had no draft to restore, nothing else changed state, so the dependent auto-save `useEffect` (gated on that same tracker) never re-armed and silently stopped auto-saving for the rest of the session; fixed by moving the tracker to `useState` so settling restoration (draft-found or not) is itself a dependency the auto-save effect reacts to. A second, related fix: the debounced persist handler originally chained `.then()` directly on the (possibly test-mocked, non-Promise) return value of the save/delete call, which threw for any caller not returning a real `Promise`; rewritten as an `async` function using `await`, which tolerates a non-Promise return the same way production code should. RTL covers the full round-trip (restore-on-load, debounced auto-save, delete-on-clear, per-symbol draft isolation via the watchlist, watchlist add/remove); backend acceptance tests (`WorkspaceDraftAcceptanceTests`, `WatchlistAcceptanceTests`) cover the same round-trip plus per-user isolation over a real PostgreSQL, structurally identical to `TrackRecordAcceptanceTests` — **they could not be executed in this sandbox** (no Docker daemon reachable here), the same sandbox limitation named throughout this session (ADR-064/065/066/069/070/071); they are gated by the same `TestDocker.SkipIfUnavailable()` every other acceptance test in this suite already uses, so they run automatically once CI or a developer's machine provides Postgres | #226 | Implemented, unit/RTL-tested; **backend acceptance tests written but unexecuted in this sandbox** (no Docker daemon available here) |
| REQ-068 | Publication-grade chart export (#227): `AnnotatedChartComposer`'s colours were extracted into a `ChartPalette` (Dark/Light, selected by `AnnotatedChartInput.Theme`) and every fixed-pixel margin/font size is scaled by `input.Width / 1200` (the original design width) rather than hardcoded, so the composer needed zero theme- or size-specific branching in its draw steps — the same draw pipeline just asks for a palette and a scale factor. The default canvas grew from 1200×700 to the AC2 publishing minimum **1920×1080**, with an optional `scale2x` doubling to 3840×2160. A **watermark** (low-opacity, centred near the bottom) and a **disclaimer line** in the title block ("Not financial advice…") are new, always-present-when-requested draw steps. `GET /api/analyses/{id}/chart.png` gained four optional query params — `theme` (dark/light), `axisScale` (linear/log), `scale2x` (bool), `watermark` (≤64 chars) — each validated to a `Results.Problem` 400 on an unrecognized value; omitting all of them (AC4) still returns a valid, larger/nicer PNG than before, since AC4's "backward compatible" is read as the route/auth/ownership contract, not a pixel-frozen output (the entire point of the ticket is to upgrade that default look). **A hard, named scope boundary (see ADR-072)**: a saved analysis persists no pivots, so degree-notated nested labels and the live chart's true forward-projection branches (computed from the prior leg) cannot be reconstructed for export — AC1's "bands … and branches matching the live chart's data" is met for confluence-style bands and for a *reasonable, honestly-different* branch analogue (each persisted alternate scenario's own entry/target zone, drawn subordinate/low-opacity via a new `AnnotatedChartInput.AlternateZones`), not for the wave-tree/degree/leg-retracement pieces, which are structurally impossible without a much larger persistence change (storing raw pivots at save time) deliberately left out of this ticket's scope. Determinism (AC3) holds: `Compose` is still a pure function of `AnnotatedChartInput`, verified both at the composer level (same wide input twice → identical `Ops`) and the service level (same analysis + options + a fixed clock → byte-identical PNG) | #227 | Implemented; **degree-notated labels and true leg-based projection branches are an explicit, out-of-scope gap for saved-analysis export** (no persisted pivots to derive them from) |
| REQ-069 | Narrative language preference, DE/EN (#228): `AppUser` gains a single nullable `NarrativeLanguage? NarrativeLanguage` property — null means "never explicitly chosen" (distinct from an explicit English choice), rather than a new collection-shaped table, since this is one scalar per user, unlike `UserApiKey`/watchlist/drafts. **`INarrativeLanguageProvider.GetCurrentAsync()`** (HttpContext-resolving, mirrors `UserAwareChatClient`'s "open a short DI scope from a caller with no `userId` parameter" precedent) serves the eight narrator/ranker classes that have no `userId` in scope; **`INarrativeLanguageSettingsService.GetAsync(userId)/SetAsync(userId, language)`** is direct persistence CRUD, used by the settings endpoint and by `PersonaAnalystPanel` directly (it already receives `userId` as a parameter, so resolving via HttpContext would be a redundant round-trip). A single pure static helper, **`NarrativeLanguageDirective.For(language)`**, returns `null` for English (nothing to append) or a fixed German instruction string, appended additively to a system prompt — every one of the eight wired classes (`AutoWaveRankRunner`, `LlmAutoWaveAnalyzer`, `EnsembleAutoWaveAnalyzer`, `PersonaAnalystPanel`, `LlmWaveAnalyzer`, `LlmAnalogNarrator`, `LlmSentimentNarrator`, `LlmPositionNarrator`, `LlmHypothesisProposer`) calls the exact same two-line pattern already established by `AutoWaveRankRunner`'s persona-guidance append (#184/REQ-064). **AC1/AC2 needed zero signature changes to either literal `*PromptBuilder*.cs` file** (`AutoWaveAnalysisPromptBuilder`, `WaveValidationPromptBuilder`): both builders' entire output becomes only the user chat message, while the system message is always a separate hardcoded string built by their Infrastructure-layer caller — so the directive is appended only at that system-message construction site, leaving both pure static builders untouched. `EnsembleAutoWaveAnalyzer` resolves the caller's language **once** per ensemble call, reused across every provider (not re-resolved per provider), since it is the same caller's preference throughout. **AC3 required zero production-code changes**: `AnalogFactGuard`/`PositionFactGuard`/`SentimentFactGuard` already match cited figures via a language-agnostic numeric regex, never inspecting surrounding words, so German prose citing the same report figures passes identically to English and a hallucinated German figure is rejected identically — confirmed by six new German-prose regression tests, not by any guard change. **AC4** ("defaults sensibly from locale, persists per user"): the server never guesses a locale (it has no browser context); a frontend hook (`useNarrativeLanguage`) instead computes a `navigator.language`-derived suggestion the first time it learns the server value is `null`, and PUTs it to the server exactly once (a `useRef` guard prevents re-firing) — from that point on the preference behaves like any other explicitly-set value. New endpoints `GET/PUT /api/settings/narrative-language`. Backend: 853 passed / 0 failed / 157 skipped (20 new tests); frontend: 300 passed / 0 failed (5 new tests), `tsc --noEmit` and `npm run build` clean. Acceptance tests (`NarrativeLanguageAcceptanceTests`) are structurally identical to every other `*AcceptanceTests` file and gated by `TestDocker.SkipIfUnavailable()` — **written but unexecuted in this sandbox** (no Docker daemon reachable here), the same limitation named throughout this session | #228 · ADR-073 | Implemented, unit/acceptance-tested; **backend acceptance tests written but unexecuted in this sandbox** (no Docker daemon available here) |
| REQ-070 | Bug fix (found running #228's acceptance suite in CI, unrelated to #228 itself): the `/api/auth/login` rate-limit policy used a single **global** fixed-window bucket (5 req/min, no partition key) shared by the *entire* application instance — any real deployment with more than 5 total logins across all users in a given minute, or a single acceptance-test fixture creating several fresh test accounts (`WatchlistAcceptanceTests` creates ~9), would exhaust it and start silently rejecting further logins with 429. Two test helpers (`AcceptanceWebApplicationFactory.AuthenticateAsync`, and the `NewUserClientAsync` helper duplicated in `WatchlistAcceptanceTests`/`NarrativeLanguageAcceptanceTests`/`WorkspaceDraftAcceptanceTests`) never checked the login response's status code, so a rejected login left the `HttpClient` silently unauthenticated — the resulting failures surfaced several steps downstream as a confusing `401 Unauthorized` or (for a response with no body) a `JsonReaderException` on an empty payload, not as a clear "login failed" message. **Fix**: raised the `login` policy's `PermitLimit` from 5 to 60 (still a single global bucket — kept simple, matching the policy's existing global-not-partitioned design) to give realistic headroom for both concurrent production logins and test-account volume; every affected test helper now calls `EnsureSuccessStatusCode()` on the login response so any future exhaustion (or other login failure) fails immediately and legibly at its source instead of several calls later | #226/#228 (CI) · ADR-074 | Fixed |
| REQ-071 | Bug fix (found via a manual hands-on walkthrough of #219/#223's forward projection paths, unrelated to any single ticket): `pathFrom()` in `projectionPath.ts` only ever read `levels.targetZones[0]` when building a branch's projection box, silently dropping any branch whose `WaveLevels` resolves to a **retracing** wave (2, 4, or a corrective B) — those only ever carry a `SupportZone`, never a `TargetZones` entry. Concretely: at "Wave 3 unfolding," the deterministic `Speculative()` continuation correctly resolves to `Wave4()` (a real, Fibonacci-correct support zone, 23.6–38.2% of Wave 3) — but the box for it never rendered, making it look like "one of the two scenarios is missing" even though the backend had computed it correctly. **Fix**: `pathFrom` now falls back to `levels.supportZone` when there's no target zone, so every wave-specific projection the backend already computes (each with its own Elliott-correct ratios and invalidation rule — Wave 2/4/B's own retracement bands, Wave 3/5/C's own extension targets) reaches the chart, not just the subset that happens to be target-shaped. A second, related finding from the same walkthrough: the box's *time window* was anchored purely to the last placed pivot's own date — once a wave has been unfolding longer than the derived pace (`deriveProjectionTimeWindow`) suggests, the whole box could render **behind** the current price/time, reading as if the projection were already in the past. Fixed by anchoring the time window (not the connector line's own start, which stays at the real pivot) at `max(pivotDate, mostRecentCandleDate)` — a box is never drawn wholly in the past relative to the chart's own last candle. Pure, unit-tested (`projectionPath.test.ts`): a support-only branch now renders; a branch with neither zone still drops; a target zone is preferred when a branch improbably has both; the window anchors from "now" only when that's later than the pivot | #219/#223 (follow-up) · ADR-075 | Fixed |
| REQ-073 | Reliable, keyed market-data provider replacing the keyless Yahoo Finance/CoinGecko split (#170), also closing the crypto-intraday gap ADR-022 named as a deferred follow-up (#94): **`TwelveDataMarketDataProvider`** implements both `IMarketDataProvider` (daily) and `IIntradayMarketDataProvider` (hourly, feeding the existing `CandleResampler.ToFourHour` 4H path unchanged) for **both** equities/indices/ETFs and crypto pairs — one keyed source instead of two keyless ones. Friendly aliases (`NASDAQ`→`IXIC`, `SP500`→`SPX`, `BTC`→`BTC/USD`, `ETH`→`ETH/USD`) mirror the prior Yahoo alias map; any other ticker passes through, so `ISymbolResolver`-resolved instruments (unchanged — still Yahoo's search endpoint, a separate concern from OHLCV data) chart directly. **Honest degradation (AC2/AC4)**: `Supports`/`SupportsIntraday` report `false` when no API key is configured, so `TechnicalAnalysisService`'s existing "no provider supports X" `ArgumentException` fires — never an unauthenticated call — and a JSON-level Twelve Data error is classified by its status code: an availability failure (401/403/429/5xx — rate-limited, unauthenticated, upstream fault) re-throws as `HttpRequestException`, reaching `MarketDataEndpoints`' **already-existing** "market data provider is currently unavailable — try again later" 502 handler unchanged (AC2 was already met at the endpoint layer; this provider is the first to actually exercise that path via a JSON-level error, not just a transport-level one); a request-level failure (e.g. unrecognized symbol, code 400) logs and returns an honest empty result instead, the same shape every provider already uses for "no data" (AC3). No proactive lookback-window `MarketDataRangeException` (unlike Yahoo's real, documented 730-day cap) — Twelve Data's actual per-plan history depth is an unconfirmed assumption until a real subscription is chosen, so a request past it is left to the upstream's own error response rather than a guessed number. API key read from `MarketData:TwelveData:ApiKey` (empty by default, same secrets-never-committed convention as every other provider key, AC4). **Unrelated bug found and fixed in the same pass**: `GET /api/market-data/{symbol}`'s own `days` validation was hardcoded to a 365-day (1-year) ceiling, silently rejecting the frontend's own "3Y"/"5Y" range buttons (#164) with a 400 — raised to 1825 days (5 years), matching `WaveAnalysisEndpoints`' existing `LookbackDays` clamp. Unit-tested (`TwelveDataMarketDataProviderTests`, HTTP boundary stubbed — string-typed OHLCV fields, newest-first-to-ascending sort, malformed-row skip, alias mapping, both error-classification branches); `MarketDataAcceptanceTests` gained the 5-year-range regression pair (Docker-gated, written but unexecuted in this sandbox, same limitation named throughout this session). **AC1 (sustained/concurrent load test against the real provider) could not be exercised in this sandbox** — no real Twelve Data subscription/key is available here; the existing `#199` load/soak harness already exercises the *shape* of concurrent market-data load against the fake provider, and will exercise the real one once a key is configured in CI | #170, #94 (crypto-intraday half) · ADR-077 (extends ADR-022) | Implemented; **AC1 (real sustained-load test) unverified in this sandbox — no live Twelve Data key available here** |
| REQ-074 | Design-system primitive adoption: a Claude-Design handoff bundle for this product was reconciled against the live codebase before any code changed — the token layer (`tokens/colors.css`/`typography.css`/`effects.css`) turned out to already be ~100% adopted (same oklch values, same Geist/Geist Mono stack, same radius/shadow scale — confirmed byte-for-byte, not assumed), and the bundle's own `styles/components.css`/`coach.css` were a much older, smaller snapshot of the app (258/118 lines vs. the live app's 1745/1738 lines, predating most of the AI-differentiator and analyst-UX panels) — so applying either wholesale would have been a regression, not an upgrade. The bundle's genuinely new, safely-adoptable piece was its five `core/` primitives (`Button`, `Field`, `Checkbox`, `Segmented`, `Switch`) — until now the app applied their CSS classes (`.btn-primary`, `.field`, `.check`, `.seg`/`.tf-select`/`.mini-seg`, `.switch`) ad hoc via raw `className` strings in JSX, not through a shared component. Ported as typed `.tsx` (the bundle ships `.jsx` + `.d.ts`) into `frontend/src/components/core/`, then adopted at every real call site that used the underlying classes: `LoginForm.tsx` (Segmented login/register switch, Field × 3, Checkbox × 3, Button × 4 incl. the Google OAuth link via `as="a"`), `ConsentBanner.tsx` + `SettingsPage.tsx` (Switch × 6, Segmented × 3, Button × 6), `WaveWorkspace.tsx` (the timeframe + range `Segmented` selectors), `AutoAnalysisPanel.tsx`/`CoachPanel.tsx`/`LiveVerifyPanel.tsx`/`PersonaPanel.tsx` (Button, one call site each). Deliberately **out of scope**, each requiring the user's own explicit sign-off before proceeding: the bundle's new "Panel v2" bounded-height workspace layout (`AnnotationChips`/`SessionRail`, unbuilt in this app and not accounting for the now much larger set of live panels — scanner, personas, sentiment, backtest, depot, etc. — the bundle never saw); the bundle's `BrandLogo` (an unrelated "myDevTime" logo from a different repository, requested in a different design session — this app ships no logo today and none was added); and the bundle's "coach reflects, never grades" content-tone guideline, which no longer matches the product's own persona-ranking features and was left untouched rather than silently rewriting existing copy | Design handoff (no issue number) · ADR-078 | Implemented (token sync: no-op, already current; five primitives adopted at all discovered real call sites; Panel v2 layout / brand logo / content-tone explicitly deferred pending user decision) |
| REQ-072 | Second-order ("one more step ahead") forward projection (#166 follow-up, user-confirmed scope: exactly one further step, visually distinguishable, bullish/bearish treated symmetrically): `ProjectionService.Branches` gains `SpeculativeNext`/`AlternateNext` — each derived by applying the **same** "complete the current wave at its own zone edge, then re-project" mechanism (`AdvanceOneWave`, generalised from the prior single-step `Speculative()` helper) a **second, and only a second, time** onto the already-once-advanced chain, dispatched through the exact same `Motive ? Project : ProjectCorrective` rule `Resolve()` already uses for the alternate reading — so a second-order box still carries its own wave-stage-correct Elliott ratios and invalidation, never a generic guess. Bounded by construction: `AdvanceOneWave` is called at most twice per chain (never a cascade to structure completion), and each `*Next` field is null whenever its own first-order branch is null or has no zone edge to complete at. Frontend: `branchesToProjectionPaths` chains a second `ProjectionPath` (`order: 2`) onto the box the first-order path (`order: 1`) already ends at — time-anchored at that first box's own `toTimeMax`, price-anchored at its zone midpoint — for both the speculative *and* the alternate chain identically (same `secondOrderPathFrom` helper, no bullish/bearish branching). `ProjectionPathPrimitive` renders an `order: 2` path at reduced opacity (`×0.55`) so it never visually competes with the first-order box it was chained from, while staying dashed like every other subordinate (non-promoted) path. A promoted (#220) second-order box is never itself drawn as "confirmed" (`promoted` stays `false`) — a projection of a projection doesn't confirm just because the first step did. Pure, deterministic, scale-invariant (metamorphic-tested); the LLM is never in this path. Backend: 4 new tests in `ProjectionServiceTests` (33/33 passing; full suite 857/0/157/1014). Frontend: 4 new tests in `projectionPath.test.ts` covering both chains, the "no `*Next`" drop, the "no chain point" drop, and the promoted case (308/308 passing); `tsc --noEmit` clean | #166 (follow-up) · ADR-076 (extends ADR-039/ADR-041/ADR-075) | Fulfilled |
...
## ADR-069: Momentum & Volume Confirmation — Compute Indicators Once, Outside the Beam Search

**Context:** #224 wants RSI/MACD momentum divergence and a volume signature (wave 3 vs wave 5, or wave B vs A/C) surfaced as guideline rows, wired into both the manual Verify path and the deterministic candidate-generation path the "magic button" and persona panel share. The obvious first design — call the two new checkers from inside `ElliottRuleChecker.Check` (already the single hard-rule entry point) — was rejected once the actual call graph was traced: `WaveGrammarParser`'s beam search calls `CheckRules` (which dispatches to `ElliottRuleChecker.Check` for an Impulse) from `EvaluatePartition`, an inner loop that can run many thousands of times for a single parse (`ctx.Options.MaxEvaluations`). Computing an RSI/MACD series inside that loop — even a cheap one — would multiply real cost by the partition count, a performance regression this ticket has no business introducing.

**Decision:**

- **Indicators are computed exactly once per candidate-generation call, outside the parser entirely.** `WaveCandidateGenerator.GenerateParsed` gained two new optional parameters (`candles`, `indicatorCalculator`); when supplied, RSI/MACD are computed a single time before the results loop, then the two guideline checkers run once per *already-generated* top-level candidate (at most `MaxCandidates = 6`), not once per beam-search partition. The parser itself (`WaveGrammarParser`) is untouched.
- **The two checkers stay pure and static** (`MomentumDivergenceChecker`, `VolumeGuidelineChecker`), mirroring `ElliottRuleChecker`/`ZigzagRuleChecker`'s existing shape: no DI dependency of their own, callers pass already-computed data. This keeps them exhaustively hand-testable (TDD, per the issue's own governance line) independent of whatever indicator-calculation strategy the caller uses.
- **A failed guideline penalizes score the same way a parser-internal one already does** — `WaveCandidateGenerator` reapplies `WaveScoringOptions.GuidelinePenalty` per failed guideline after the fact, deliberately mirroring `WaveGrammarParser.EvaluatePartition`'s own `for (failedGuidelines) score *= GuidelinePenalty` loop rather than inventing a second scoring formula. `WaveVerifier.ScoreOrNull` (the manual-verify score path) does **not** apply this penalty loop today, for any guideline — a pre-existing asymmetry this ticket does not fix, named here rather than silently left unaddressed.
- **Only two shapes are "understood"**: a 6-pivot impulse (wave 3 vs wave 5) or a 4-pivot ABC correction (wave A vs wave C) — selected by the top-level `StructureKind` (Impulse/Diagonal → 6-pivot; Zigzag/Flat → 4-pivot), explicitly excluding Triangle even though its 6 pivots would coincidentally match the impulse checker's pivot count — a Triangle's five legs (A-B-C-D-E) mean something structurally different from an impulse's five waves, and mislabeling them would produce a nonsensical "wave 3/wave 5" narrative.
- **Wired into exactly the two "live" paths the issue's own motivation names** — manual Verify (`WaveVerificationService` → `WaveVerifier.Verify`) and Auto-analyze/persona-panel candidate generation (`AutoWaveAnalysisService`/`PersonaPanelAnalysisService` → `WaveCandidateGenerator.GenerateParsed`) — leaving six secondary `GenerateParsed` callers (the scanner, backtest engine, top-down analyzer, historical-analog service, chart-verification assembler, setup-history builder) unwired by default, since the new parameters are optional and default to skipping the guidelines entirely. A deliberate scope boundary, not an oversight: those paths are investigative/aggregate features, not the "count workflow" #224 is about.
- **AC5 needed zero prompt-builder changes.** `WaveValidationPromptBuilder`/`AutoWaveAnalysisPromptBuilder` already iterate every `WaveRuleReport.Rules` entry under an "AUTHORITATIVE — already computed, do NOT re-derive" header — the two new guideline rows ride that existing mechanism for free, inheriting the same "the LLM may cite, never compute" contract every other rule row already has.

**Consequences:**

| | |
|---|---|
| (+) | Zero performance impact on the beam search — verified by construction (indicators are computed outside `WaveGrammarParser` entirely, not just "made cheap") rather than by a benchmark this session couldn't run reproducibly anyway |
| (+) | AC5 (fact-guard framing for the LLM prompt) is satisfied by an existing mechanism, not new code — one less surface to get wrong |
| (+) | The two checkers are trivially unit-testable in isolation (`MomentumDivergenceCheckerTests`, `VolumeGuidelineCheckerTests`) — no chart, no DI container, no candle-fetch mock needed |
| (-) | `WaveVerifier.ScoreOrNull`'s pre-existing "no guideline penalty applied" gap is named, not fixed — a saved manual-verify score does not reflect a failed momentum/volume guideline the way an Auto-analyze/persona-panel candidate's score does |
| (-) | Six secondary `GenerateParsed` call sites (scanner, backtest, top-down, analogs, chart-verification, setup-history) never see these guidelines — an explicit, bounded scope decision, not a silent gap |
| (-) | The RSI sub-pane is the first use of lightweight-charts v5's multi-pane API (`addSeries(..., paneIndex)`, `removePane`) in this codebase — verified against the library's own TypeScript definitions (no live-browser verification was possible in this sandbox), so the pane's actual on-screen behavior is unverified beyond the component-level `not.toThrow()` tests |

---

## ADR-070: Pivot Drag-to-Move — Hand Hit-Testing Against the Wave Line, Suppression Instead of Prevention

**Context:** #225 is the explicitly-deferred remainder of #165: pivots can be placed (click) and fine-tuned (nudge), but not dragged. The design has to resolve two problems lightweight-charts v5 doesn't solve for you: (1) the chart API has no marker-drag support and no pointer-level event subscription at all (only `subscribeClick` and `subscribeCrosshairMove`), so "which pivot is under the pointer" and "the pointer is moving" both need to be worked out by hand; (2) a drag's release is, physically, the same gesture as a click — lightweight-charts will report it as one via `subscribeClick` — so click-to-place (#159/#160) and drag-to-move will collide unless something explicitly tells them apart.

**Decision:**

- **Hit-testing reuses the `user` wave line, not a new data structure.** `PriceChart` already receives `waveLines` (time+price pairs) for rendering the connected polyline through the analyst's own pivots — the marker layer (`ChartMarker[]`) only carries a date, not a price, so it can't answer "how far is this pointer, in pixels, from pivot N". Converting each wave-line point to a pixel coordinate via `chart.timeScale().timeToCoordinate` + `series.priceToCoordinate` and taking the nearest one within a fixed radius (14px) is a few lines against data the component already has, rather than plumbing a new marker-position side-channel through the chart.
- **Raw DOM `pointerdown`/`pointermove`/`pointerup` on the chart container**, not a chart-level API, since none exists. Chart panning/scaling (`handleScroll`/`handleScale`) is explicitly disabled for the drag's duration and restored on release, so the pivot drag and the chart's own click-to-pan behavior never fight over the same gesture.
- **AC2 (a drag must never spawn a new pivot) is solved by suppression, not prevention.** There is no way to tell lightweight-charts "don't fire your click handler for this gesture" — it decides that internally. Instead, `endDrag` sets a one-shot `justDraggedRef` flag; the existing `subscribeClick` handler checks and clears it first, before doing anything else. This is a narrow, well-understood pattern (the exact "next event is a byproduct of the last one, swallow it once" shape), not a broad "ignore clicks while X" guard that could accidentally eat an unrelated later click.
- **The feature is opt-in per `PriceChart` instance** (`onPivotDragEnd` unset ⇒ no pointer listeners engage beyond a single early-return check) rather than always-on, so every other consumer of `PriceChart` (none of which pass a `user` wave line meant to be draggable) is provably unaffected.
- **A drag-preview state, kept deliberately separate from the committed `annotations`.** AC3 asks for the leg readout to update live during the drag but the debounced re-verify to fire once, on drop. Rather than debounce differently or add a second verify path, `WaveWorkspace` introduces `displayAnnotations` — `annotations` with the in-flight drag substituted in — and feeds only the *rendering* consumers (leg readout, markers, the `user` wave line) from it; the live-verify `useEffect` keeps watching the untouched `annotations`, so it is structurally impossible for a pointermove to trigger a network call.

**Consequences:**

| | |
|---|---|
| (+) | Zero new state in `PriceChart` beyond drag bookkeeping — hit-testing rides the same `waveLines` prop every consumer already passes for rendering |
| (+) | The suppression flag is a single boolean, set and cleared in the same gesture, not a broader "drag mode" that could linger and eat a later, unrelated click |
| (+) | Existing #165 machinery (`snapToCandle`, the 400ms debounced live-verify, `legMeasurements`) is reused completely unmodified — the drag path converges on the exact same commit shape the nudge path already uses |
| (-) | **AC4 (hands-on verification in a real browser, including basic touch support) could not be performed in this session** — no live browser or composed stack was available, the same sandbox limitation named throughout this session (ADR-064/065/066/069). AC1–AC3 are unit-tested (pointer-event simulation via `dispatchEvent`, asserting the callback contract), but the feature's actual on-screen feel is unverified. The issue's own governance line states hands-on verification is required before merge — this ADR states plainly that requirement is **not yet met**, rather than claiming it is |
| (-) | The 14px hit-radius and drag-vs-pan disable/restore were chosen from reading the library's API surface, not tuned against a real pointer/touch device — likely needs adjustment once tested hands-on |

---

## ADR-071: Workspace Drafts — Opaque JSON Rows, State (Not a Ref) Gates Auto-Save

**Context:** #226 asks for two related but separable things: (1) a per-`(symbol, interval)` draft of in-progress work that auto-saves and auto-restores, distinct from a deliberate track-record save, and (2) a user-managed watchlist replacing the hardcoded quick-symbol row. Both need per-user server-side persistence (AC2 — "survives a reload and another device"), so both follow the existing `AnalysisSnapshot`/`TrackRecordService`/`TrackRecordEndpoints` shape (EF entity → service scoped by `userId` → minimal-API group under `RequireAuthorization()` + `RequireRateLimiting("per-user")`) rather than inventing a new persistence pattern.

**Decision:**

- **`WorkspaceDraftRow` stores annotations and settings as opaque JSON strings, not typed columns.** Unlike `AnalysisSnapshot` (whose fields feed live outcome evaluation and must be interpretable server-side), nothing on the backend ever reads inside a draft's contents — it is saved and returned verbatim. Typed backend DTOs (`WorkspaceDraft`, `WorkspaceDraftSettings`) exist for the wire contract and OpenAPI docs, but the EF row itself just round-trips `JsonSerializer`-produced strings. This avoids a second, backend-owned copy of every frontend UI-state field (count type, layer toggles, oscillator, log scale, sub-wave depth) that would need updating every time the frontend adds one.
- **No annotations ⇒ delete the draft, never save an empty one.** The issue's own motivation is "glance at Silver, come back" — a symbol merely viewed, never annotated, should leave no trace, so the 50-draft LRU cap (AC-adjacent, from the wish list) is spent only on genuine in-progress work.
- **The watchlist seeds the four legacy quick symbols (SP500/NASDAQ/BTC/ETH) lazily, on a new user's first `ListAsync` read**, rather than at account-creation time — this is the natural place the old hardcoded array's job moves to, and it means an account created before #226 shipped is seeded identically to a brand-new one, with no backfill migration needed.
- **`WatchlistEntry.LastPrice` reuses `IQuoteProvider`**, the same seam `DepotEnrichmentService` already uses for position pricing (`CachingQuoteProvider` decorating `TechnicalAnalysisQuoteProvider`) — no new external call shape, and its documented "never throws, null when unavailable" contract means one bad symbol degrades that one entry's price to blank rather than failing the whole list.
- **Restore-completion tracking is `useState`, not a `useRef`** (a bug found by the frontend RTL tests written for this ticket, not by inspection — see REQ-067's "Bug found by TDD" note). A ref mutation doesn't itself schedule a re-render, so when a symbol has no draft to restore, nothing else changes state and a `useEffect` gated on that ref's value never re-evaluates — auto-save silently stopped working for the rest of the session after the first symbol with no prior draft. Moving the tracker to state makes "restoration settled for this key" itself a reactive dependency, so the auto-save effect correctly re-arms whether or not there was anything to restore.

**Consequences:**

| | |
|---|---|
| (+) | Zero backend coupling to frontend UI-state shape beyond the thin `WorkspaceDraftSettings` DTO — a new frontend toggle needs no new migration |
| (+) | Reuses `TrackRecordEndpoints`'s exact auth/rate-limit/ownership shape and `IQuoteProvider`'s exact seam from depot enrichment — no new persistence or pricing pattern introduced |
| (+) | The ref→state fix is verified by the very RTL test that caught the bug (auto-save asserted to fire after a null-draft restore), not just inspection |
| (-) | **Backend acceptance tests (`WorkspaceDraftAcceptanceTests`, `WatchlistAcceptanceTests`) could not be executed in this sandbox** — no Docker daemon is reachable here, so the real-PostgreSQL round-trip and per-user isolation they assert are unverified until CI or a developer's machine runs them. They are written to the exact same pattern as every other `*AcceptanceTests` file in this suite (Testcontainers or `ACCEPTANCE_PG_CONNSTRING`, gated by `TestDocker.SkipIfUnavailable()`), so they are believed correct by construction, not proven by a green run |
| (-) | "Glance at a symbol" vs. "started working" is drawn at zero-annotations, a simple but coarse line — placing one pivot then immediately clearing it still creates and then deletes a draft row within the same debounce window, a harmless but slightly wasteful round-trip |

---

## ADR-072: Publication-Grade Chart Export — Palette Extraction, Proportional Scaling, and a Named Wave-Tree Gap

**Context:** #227 asks the exported PNG (`GET /api/analyses/{id}/chart.png`) to reach visual parity with the live chart: shaded bands, degree-notated labels, projection branches, theme/size options and a watermark. The renderer (`AnnotatedChartComposer` → `ChartScene` → `SkiaAnnotatedChartRenderer`) already had the right shape for this (ADR-026's backend-agnostic draw-op seam), but `AnalysisChartService.BuildInput` assembles its `AnnotatedChartInput` from a **saved** `TrackedAnalysis`, and a saved analysis persists only flat geometry (invalidation, entry/target zones, a shallow scenario tree) — never the raw pivot sequence a live count's `WaveNode` tree or `ProjectionService.Branches` are built from. Any implementation has to decide, explicitly, what "parity" can mean for data that was never captured.

**Decision:**

- **Extract the composer's colours into a `ChartPalette` (Dark/Light), selected by a new `AnnotatedChartInput.Theme`.** The composer's own layout logic is completely unaware of themes — it asks for a palette once at the top of `Compose` and threads it through; adding a third theme later is a third `ChartPalette` static instance, not a new code path.
- **Scale every fixed-pixel layout constant by `input.Width / 1200`** (1200 being the original design width the margins/fonts were tuned against), rather than hardcoding a second layout for the new 1920×1080 default. This is a single multiplier applied at the top of `Compose` — it keeps the scene byte-identical to before at exactly 1200px (so the entire existing `AnnotatedChartComposerTests` suite needed zero assertion changes) and grows cleanly for 1920×1080 or the `scale2x` 3840×2160 variant, with no per-size branching anywhere in the draw steps. Stroke widths were deliberately left unscaled (several tests assert exact `StrokeWidth` values, e.g. channel rays at `1.5f`) — line *weight* staying constant while margins/fonts grow is also the visually correct choice; only positions and text sizes need to track the canvas.
- **A saved analysis's "projection branches" are its own persisted alternates, not a recomputed live branch.** `TrackedAnalysis.Scenarios` already carries each alternate's own entry/target zone (`AnalysisScenarioRow`, unchanged). `BuildInput` now maps every non-retired alternate to a pair of `PriceZone`s in a new `AnnotatedChartInput.AlternateZones`, and the composer draws them at a visibly lower opacity than the primary's own zones with an "(alt)" suffix already baked into the label. This is a different thing from the live chart's forward-projection branch (which extrapolates from the prior leg's geometry) — it is deliberately named as such here and in REQ-068, not silently substituted as if it were the same feature.
- **Degree-notated nested labels and the live chart's specific leg-based branch/retracement math are named out of scope for saved-analysis export, not attempted.** Both need the pivot sequence — `treeToDegreeMarkers` walks a `WaveNode` tree; `ProjectionService.Branches`'s retracement % is measured against the prior leg. Neither exists once an analysis is saved (see REQ-068's flat-persistence trace). The alternative — persisting a compact pivot/tree snapshot alongside the flat fields at save time — is a real, larger option, but is a `TrackAnalysisRequest`/`AnalysisSnapshot` schema change in its own right and was deliberately left for a follow-on ticket rather than folded into this one under time pressure.
- **AC4 ("stays backward compatible") is read as a contract-compatibility, not pixel-compatibility, requirement.** Every new option is optional with a default that reproduces the pre-#227 semantics (dark theme, linear axis, no watermark); the only default that visibly changed is the canvas size (1200×700 → 1920×1080), which is the ticket's entire point — a caller passing zero query params still gets a 200, `image/png`, ownership-scoped response, just now publish-sized.

**Consequences:**

| | |
|---|---|
| (+) | Existing `AnnotatedChartComposerTests` needed no assertion changes — the palette/scale refactor is provably behavior-preserving at the original 1200px design width |
| (+) | New composer-level tests assert the palette swap, watermark opacity, alternate-zone subordination and disclaimer text purely structurally (`ChartScene.Ops`), matching this file's existing no-golden-image convention; service-level tests decode the produced PNG's actual dimensions and a background pixel (via `SKBitmap`) to prove the wiring reaches the real renderer, not just the pure composer |
| (+) | Determinism (AC3) is verified at two levels: `Compose` called twice on the same wide input yields identical `Ops` (composer-level), and `RenderChartAsync` called twice with the same analysis, options and a fixed `TimeProvider` yields byte-identical PNGs (service-level) |
| (-) | **Degree-notated labels and true leg-based projection branches are not in the export for a saved analysis** — a real, acknowledged gap against AC1's literal "matching the live chart's data" wording, not silently dropped: named here, in REQ-068, and left as a candidate for a follow-on ticket that would need to persist pivot data at save time |
| (-) | New acceptance tests for the export's query-param validation (`GetChart_WithExportOptions_ReturnsOk` etc., in `AnalysisChartAcceptanceTests`) **could not be executed in this sandbox** — no Docker daemon reachable here, the same limitation named throughout this session (ADR-064 onward) |

---

## ADR-073: Narrative Language Preference — a Nullable Scalar, a Provider/Settings-Service Split, and One Directive Helper

**Context:** #228 asks every LLM-authored free-text narrative (auto-analysis rationale, validation notes, analog/sentiment/position summaries, hypothesis reasons, persona commentary) to honour a per-user German-or-English preference, defaulting sensibly from the browser locale, without ever letting the language choice touch the deterministic geometry (ADR-009 holds unchanged — the LLM still never does geometry, in any language). The codebase has no existing "coach preferences" table; the closest precedent is `UserApiKey` (per-user secret) and `AutoWaveRankRunner`'s persona-guidance append (#184/REQ-064, a system-prompt suffix pattern). Several call sites that need to honour the preference (`LlmAnalogNarrator`, `LlmSentimentNarrator`, etc.) have no `userId` parameter at all — only `UserAwareChatClient` has previously solved that shape, by resolving the current user from `IHttpContextAccessor` inside a short-lived DI scope.

**Decision:**

- **A single nullable `AppUser.NarrativeLanguage` property, not a new table.** Unlike API keys or watchlist entries, this is one scalar per user with no independent lifecycle of its own — a new EF entity/row-per-user table would be pure ceremony for a value that is conceptually a user attribute. `null` is a distinct, meaningful state ("never explicitly chosen") from an explicit `English`, which AC4's locale-suggestion-once mechanism depends on to know whether it has already run.
- **Two seams, not one, split by whether the caller already has a `userId`.** `INarrativeLanguageProvider.GetCurrentAsync()` mirrors `UserAwareChatClient`'s HttpContext-resolving pattern for the eight narrator/ranker classes that have none. `INarrativeLanguageSettingsService.GetAsync(userId)/SetAsync(userId, language)` is plain persistence CRUD, used by the settings endpoint and directly by `PersonaAnalystPanel` — which already receives `userId` as a method parameter, so routing it through an HttpContext-resolving provider instead would be a strictly worse, indirect path to the same value.
- **One pure static helper owns the entire directive string.** `NarrativeLanguageDirective.For(NarrativeLanguage)` returns `null` for English or a fixed German instruction for German; every wired call site's own logic is the identical two-line append (`if (...For(language) is { } d) systemPrompt += " " + d;`), so the wording is defined in exactly one place, not duplicated eight times.
- **Neither literal `*PromptBuilder*.cs` file gained a `language` parameter.** Both `AutoWaveAnalysisPromptBuilder.Build` and `WaveValidationPromptBuilder.Build` produce only the *user* chat message; the *system* message is always a separate hardcoded string owned by their Infrastructure-layer caller (`AutoWaveRankRunner`, `LlmWaveAnalyzer`). The directive is appended only where that system message is actually constructed, so both pure static builders — and their existing purity/signature contracts — are untouched. This reads AC1/AC2 as "every narrative-producing caller demonstrably honours the setting," not "these two specific files must grow a parameter."
- **`EnsembleAutoWaveAnalyzer` resolves the language once per call, before fanning out**, not once per provider — it is the same caller's preference for every provider in that ensemble, and resolving it per-provider would be redundant work with no behavioural difference.
- **Fact-guards needed no code changes for AC3.** `AnalogFactGuard`/`PositionFactGuard`/`SentimentFactGuard` already match cited numbers via a language-agnostic numeric regex, never the surrounding prose — confirmed, not assumed, by adding German-prose regression tests (grounded German narrative passes; a fabricated German figure is rejected) rather than touching any guard.
- **The server never guesses a locale; the frontend suggests once.** `useNarrativeLanguage` learns the server value is `null`, computes a `navigator.language`-derived default, and PUTs it exactly once (a `useRef` guard prevents re-firing on re-render) — after which the value is indistinguishable from any other explicitly-set preference. This keeps the server a pure store with no browser-context assumptions, while still satisfying AC4's "defaults sensibly from locale, persists per user" as two ends of one one-time mechanism.

**Consequences:**

| | |
|---|---|
| (+) | Zero signature changes to either pure static prompt builder, and zero changes to any public narrator/ranker interface method (`IAutoWaveAnalyzer.RankAsync`, `IAnalogNarrator.NarrateAsync`, etc.) — only new private constructor dependencies on their Infrastructure implementations |
| (+) | The directive wording lives in exactly one place (`NarrativeLanguageDirective.For`); changing it later touches one file, not eight call sites |
| (+) | Fact-guard AC3 compliance is proven by new tests, not merely asserted — the language-agnostic regex claim is now regression-pinned in German for all three guards |
| (-) | `NarrativeLanguageAcceptanceTests` (round-trip, cross-user scoping, auth) **could not be executed in this sandbox** — no Docker daemon reachable here, the same limitation named throughout this session (ADR-064 onward) |

---

## ADR-074: Login Rate Limit — Global Bucket Raised From 5 to 60 req/min, Test Helpers Assert Success

**Context:** CI's real-Postgres acceptance run (unreachable in this sandbox, so this class of bug is only ever caught in CI — see ADR-064 onward) surfaced 3 failing tests in `WatchlistAcceptanceTests`, all with symptoms consistent with "the client's login silently didn't happen": `401 Unauthorized` on a request that should have succeeded, and a `JsonReaderException` parsing an empty response body (which is exactly what `JsonDocument.Parse` throws on a bodyless 401). Tracing it back: `/api/auth/login` carries the `login` rate-limit policy (`RateLimitingExtensions.cs`), configured as a **single global fixed-window bucket** — 5 requests per minute, shared by every caller of the entire application instance, not partitioned by IP or user (unlike every other named policy, which is either per-IP or falls back to it). `WatchlistAcceptanceTests` creates a fresh test account per test (~9 logins across one fixture's lifetime, including `OneTimeSetUp`), so the 6th-and-later logins within any 60-second window were rejected with `429`. The test helpers (`AcceptanceWebApplicationFactory.AuthenticateAsync`, and an identical `NewUserClientAsync` duplicated in three acceptance-test files) never checked the login call's status code, so a rejected login left the `HttpClient` quietly unauthenticated instead of failing the test at its actual point of failure.

**Decision:**

- **Raise `login`'s `PermitLimit` from 5 to 60, keep it a single global bucket.** A cap of 5 total logins per minute across the *entire application* — not per account, not per IP — was never a realistic brute-force control for anything beyond a single-tester demo; any real deployment with a handful of concurrent users logging in would already have been hitting this. 60/min keeps a meaningful ceiling against a scripted flood while giving realistic headroom for both production concurrency and legitimate test-account volume. A true per-account or per-IP brute-force control (mirroring `per-user`'s partition-with-IP-fallback shape) is a more correct design in the abstract, but partitioning by IP would not have fixed the observed CI flakiness anyway — every acceptance-test request shares one synthetic loopback address inside `TestServer`, so a per-IP bucket is still a single bucket in this topology — and reworking the policy's shape is a larger, separable decision than this fix's scope.
- **Every acceptance-test helper that logs in a fresh user now calls `EnsureSuccessStatusCode()` on the login response.** This turns any future rate-limit exhaustion (or any other login failure) into an immediate, legible failure at the `NewUserClientAsync`/`AuthenticateAsync` call site, instead of a confusing downstream symptom several requests later that has to be traced back through a stack trace pointing at the wrong line.

**Consequences:**

| | |
|---|---|
| (+) | `WatchlistAcceptanceTests` (and any other fixture creating several fresh accounts) no longer silently loses logins to a rate limit sized for a single caller |
| (+) | A future regression here fails at the actual point of failure (`login.EnsureSuccessStatusCode()` throws with the real status code) rather than several calls downstream as an opaque 401/JSON-parse error |
| (-) | The `login` policy is still a single global bucket, not per-account/per-IP — a determined distributed brute-force attempt spread across many source IPs is throttled no better than before; a proper per-account/IP-partitioned login policy remains a legitimate follow-on, deliberately left out of this fix's scope |
| (-) | This fix could not be verified against real Postgres in this sandbox (no Docker daemon reachable here) — it relies on the next CI run for confirmation |

---

## ADR-075: Forward Projection Boxes — Draw Support Zones Too, Anchor the Time Window at "Now"

**Context:** A manual, hands-on walkthrough of the live chart's forward-projection boxes (#219's branch zones, #223's time-bounded paths) — running the real deterministic engine against a hand-crafted, controllable candle series rather than only unit tests — surfaced that at "Wave 3 unfolding," only *one* of the two expected boxes (bullish continuation, bearish alternate) ever appeared, and the one that did appear sat implausibly close to the last placed pivot rather than reflecting where the count could plausibly land. Tracing it into `ProjectionService.cs` confirmed the backend's per-wave-stage Elliott rules were never the problem — `Wave2`/`Wave4`/`CorrectionWaveB` correctly compute their own Fibonacci retracement **support** zones (50–61.8% of Wave 1, 23.6–38.2% of Wave 3, 50–61.8%/90–105% of Wave A respectively) with their own wave-specific invalidation rule, exactly mirroring `Wave3`/`Wave5`/`CorrectionWaveC`'s **target** zones. The gap was purely in the frontend's `pathFrom()`, which only ever asked "is there a target zone?" — silently discarding a correctly-computed support zone instead of drawing it.

**Decision:**

- **`pathFrom` reads `targetZones[0] ?? supportZone`, not `targetZones[0]` alone.** No change to any Elliott rule or ratio — those already differ correctly per wave stage in `ProjectionService.cs`; this only stops the renderer from throwing away half of what the backend already computed. A branch with neither zone still contributes no path (unchanged).
- **The time window is anchored at `max(lastPivotDate, mostRecentCandleDate)`, not the pivot's date alone.** The connector *line* still starts at the real last pivot (`fromTime`/`fromPrice` unchanged) — only the *window* computation (`toTimeMin`/`toTimeMax`) shifts forward when the pivot's own date has already fallen behind the chart's most recent candle. A count that keeps unfolding longer than its own derived pace (`deriveProjectionTimeWindow`, [0.5×, 1.5×] of the average leg duration) would otherwise draw a target box entirely in the past relative to current price — visually indistinguishable from a stale/wrong projection.
- **Both fixes are pure, deterministic, and covered in `projectionPath.test.ts` alone** — no backend change, no new endpoint, no change to `WaveLevels`'s shape. `WaveWorkspace.tsx` now threads the most recent candle's `openTime` through as the `now` argument at its one call site.

**Consequences:**

| | |
|---|---|
| (+) | Every wave-specific projection the backend already computes now reaches the chart — Wave 2/4/B's support zones are no longer invisible next to Wave 3/5/C's target zones |
| (+) | A long-unfolding count's projection box can no longer render behind "now" — the exact defect a manual walkthrough (not a unit test, which had only ever exercised target-shaped fixtures) caught |
| (+) | Zero blast radius: both fixes are confined to `projectionPath.ts`, a pure mapping module with no DOM/network/state — existing tests needed no changes beyond the two new fixture shapes |
| (-) | A separate, larger inconsistency was found in the same walkthrough but is **explicitly out of scope here**: at "Wave 3 unfolding" the alternate's *label* text ("Corrective B-wave") doesn't match the *geometry* actually resolved (`CorrectionWaveC`, a C-wave) — and the automatic alternate is hardcoded to a Zigzag reinterpretation, never offering a Flat/overshooting-B alternate even though `CorrectionWaveB` already supports it. Both are named, deliberate follow-on work, not folded into this fix |

## ADR-076: Second-Order Forward Projection — One Bounded Extra Step, Fainter, Symmetric

**Context:** With REQ-071/ADR-075 fixed, every wave-specific first-order projection box (bullish continuation, bearish alternate) now reaches the chart. The user, reviewing the resulting picture, asked for one further named follow-up from the #166 backlog: show what happens *after* each of those boxes resolves — e.g. the Wave 5 target that would follow a speculative Wave 4, or the full A-B-C recovery target that would follow an alternate Wave C — rather than leaving the analyst to mentally chain the next step themselves. Scope was deliberately narrowed by the user before implementation, verbatim: *"Nur einen Schritt und sollten visual unterscheidbar sein gleich behandeln"* — only one further step (not a cascade all the way to structure completion), the second-order box must be visually distinguishable from the first, and both the speculative and alternate chains must be treated by the same rule.

**Decision:**

- **Reuse, don't duplicate, the existing one-step-ahead mechanism.** The prior single-purpose `Speculative(sorted, levels)` helper is generalised into `AdvanceOneWave(sorted, levels, project)` — "complete the current wave at its own zone edge (`CompletionPrice`), append a synthetic pivot there, re-project" — parameterised over which projector to call. `Branches()` calls it once to get `Speculative`/`Alternate` (unchanged), then calls it **exactly one more time** on each resulting chain to get `SpeculativeNext`/`AlternateNext`. No loop, no recursion — the call site itself is the bound.
- **The alternate chain reuses `Resolve()`'s own dispatch rule** (`Motive ? Project : ProjectCorrective`), not a new one — so a second-order alternate box still resolves through the correct motive/corrective projector for whatever structure the reinterpretation actually is, exactly as the first-order alternate already does.
- **Both `*Next` fields are null whenever their own first-order branch is null or has no completable zone edge** — a second-order box can never appear "detached" from a first-order one that isn't itself drawn.
- **Frontend visual distinction is opacity, not colour or shape.** A dedicated `order: 1 | 2` field on `ProjectionPath` drives `ProjectionPathPrimitive` to multiply `globalAlpha` by `0.55` for `order === 2`, layered on top of the existing dashed/solid (promoted) distinction — so a second-order box is always fainter than whatever first-order box it's chained from, regardless of variant or promotion state, satisfying "visually distinguishable" without adding a third colour to the existing speculative/alternate palette.
- **The chain point is geometric, not a new backend field**: `secondOrderPathFrom` anchors the second box's connector at the first box's own `toTimeMax` (time) and zone midpoint (price) — no new anchor data needed from the backend, since the first-order path's own resolved geometry already *is* the natural continuation point.
- **Symmetric by construction**: `branchesToProjectionPaths` calls the identical `secondOrderPathFrom` helper for both the speculative and the alternate chain, with no bullish/bearish branching in the logic — satisfying "gleich behandeln" (treated identically) directly in the code shape, not just by convention.
- **A second-order box is never itself `promoted`** — `#220`'s invalidation-break promotion only ever applies to a first-order alternate; a projection chained off of it doesn't inherit "confirmed" status just because its parent step did.

**Consequences:**

| | |
|---|---|
| (+) | Analysts get one further named step ahead per chain — a materially more complete forward picture — without ever risking runaway/misleading multi-step speculation, since the mechanism is bounded to exactly one extra advance by the shape of the code, not by a runtime guard |
| (+) | Zero new Elliott-rule surface: every second-order box still carries its own wave-stage-correct ratios and invalidation, because it's produced by the *same* `Project`/`ProjectCorrective`/`Resolve` functions the first-order boxes already use — no parallel, potentially-divergent projection path was introduced |
| (+) | Visually safe by construction: a second-order box can never out-compete the first-order box it depends on, since its opacity is always strictly lower |
| (-) | `ProjectionBranches` and `ProjectionPath` both grew by two fields; every existing test fixture that constructs either literal needed updating (mechanical, but touched 3 test files) |
| (-) | The chain-anchor choice (first box's `toTimeMax`/zone-midpoint) is a reasonable default, not a backend-computed "most likely" point — a second-order box's *time* placement is necessarily softer than a first-order one's, since it compounds two derived-pace windows rather than one |

## ADR-077: One Keyed Market-Data Provider (Twelve Data) Replaces the Yahoo/CoinGecko Split

**Context:** The app's OHLCV data came from two **keyless, rate-limited** sources — Yahoo Finance's public chart API (equities/ETFs/indices, daily + hourly) and CoinGecko's free tier (BTC/ETH, daily only). This is issue **#170**'s own framing of a 1.0 reliability blocker: neither source offers any throughput guarantee, and CoinGecko's free tier has never had intraday candles at all — a gap ADR-022 named explicitly as deferred (*"Crypto intraday (BTC/ETH 1H/4H) needs CoinGecko intraday and is deferred"*), which is also what the still-open issue **#94** ("4-hour timeframe via an intraday-capable provider") was actually blocked on once the equities half shipped under #116/ADR-022. The user, asked to choose between researching a vendor themselves or getting a recommendation, asked for a recommendation and confirmed both asset classes should move to **one** provider rather than keeping the keyless sources as a fallback.

**Decision:**

- **Twelve Data** was recommended and chosen: one subscription covers equities, indices and crypto pairs, with intraday granularity down to 1 minute — closing both the reliability gap (#170) and the crypto-intraday gap (#94) with a single integration instead of two. (Alternatives considered: Polygon.io prices indices as a separate, pricier product tier from crypto/stocks; EODHD is cheaper but its intraday depth tops out at 1–5 minute bars and its index coverage is less certain.)
- **`TwelveDataMarketDataProvider`** implements both `IMarketDataProvider` and `IIntradayMarketDataProvider` on one class (mirroring Yahoo's prior dual-role shape), replacing both `YahooFinanceMarketDataProvider` and `CoinGeckoMarketDataProvider` outright — both deleted, along with their tests, rather than left dead in the tree. **`ISymbolResolver`/`YahooSymbolResolver`** (ticker/name/ISIN search) is untouched — a genuinely separate concern (instrument discovery, not OHLCV fetching) that still has no reason to move off Yahoo's free search endpoint.
- **No lookback-window pre-check** (unlike Yahoo's real, documented 730-day hourly cap): Twelve Data's actual per-plan history depth is an unconfirmed assumption in this sandbox (no live subscription to verify against), so a request past whatever the real plan supports is left to the upstream's own error response — honest deferral rather than a guessed, possibly-wrong `MarketDataRangeException` bound.
- **Honest degradation, fail-closed on a missing key**: `Supports`/`SupportsIntraday` return `false` whenever `MarketData:TwelveData:ApiKey` is unset, so `TechnicalAnalysisService`'s existing `FirstOrDefault(p => p.Supports(symbol))` selection finds no provider and raises its existing, already-tested "no provider supports X" `ArgumentException` — never an unauthenticated call reaching Twelve Data. This is a deliberate consequence: without a real key, **the app has zero market data** rather than silently falling back to a keyless source, matching #170's own framing of "one reliable provider," not "reliable provider with a flaky fallback."
- **JSON-level error classification, reusing an endpoint path that already existed**: Twelve Data returns HTTP 200 with a JSON `"status": "error"` body for rate-limit/auth failures, not just transport-level non-2xx responses. Status codes 401/403/429/5xx are classified as an availability problem and re-thrown as `HttpRequestException`, landing in `MarketDataEndpoints`' **pre-existing** `catch (HttpRequestException)` branch — already logging server-side and returning a 502 "market data provider is currently unavailable — try again later" (this branch existed before this change but had never actually been exercised by a JSON-level error, only a transport-level one). Any other status code (e.g. 400 for an unrecognized symbol) is a request-level problem, not an availability one — logged and returned as an empty candle list, the same "no data" shape Yahoo/CoinGecko already used for an unrecognized symbol.
- **Unrelated bug found and fixed in the same pass**: `GET /api/market-data/{symbol}`'s `days` parameter validation was hardcoded to a 365-day (1-year) ceiling — silently rejecting the frontend's own "3Y"/"5Y" range buttons (#164) with a flat 400, discovered when the user asked to see 5 years of data. Raised to 1825 days (5 years), matching `WaveAnalysisEndpoints`' existing `LookbackDays` clamp bound — the two endpoints' day-range validation is now consistent.

**Consequences:**

| | |
|---|---|
| (+) | One reliable, keyed, paid provider instead of two keyless/rate-limited ones — directly closes #170's named reliability gap |
| (+) | Crypto gains intraday (1H/4H) support for the first time — closes the gap ADR-022 explicitly deferred, and is the actual remaining substance of issue #94 |
| (+) | Zero dead code: both replaced providers and their tests are deleted outright, not left unregistered |
| (+) | The 5-year range bug fix is small, isolated, and unlikely to regress anything — the endpoint's own upper bound simply now matches what the frontend has offered since #164 |
| (-) | **AC1 (a real sustained/concurrent load test against the actual provider) could not be exercised in this sandbox** — no live Twelve Data subscription/key is available here; the existing #199 load/soak harness already exercises the *shape* of concurrent market-data load against the fake test provider and will exercise the real one once a key is configured in CI |
| (-) | The app now has **zero market data without a configured Twelve Data key** — a deliberate honesty tradeoff (see Decision), but a stronger single point of failure than the previous two-source split, until the key is actually provisioned |
| (-) | The hourly-history depth assumption (no proactive `MarketDataRangeException`) is unverified against a real plan — if Twelve Data's actual depth is much shorter than expected, the honest-empty-result path (not a crash) still holds, but the exact boundary is unconfirmed |

## ADR-078: Design-System Handoff — Reconcile Before Applying, Adopt Only What's Actually New

**Context:** A Claude Design handoff bundle ("Elliott Wave Analyzer Design System") was supplied with an instruction to adopt it fully across the frontend. Its own bundled `README.md` explicitly warns coding agents to read everything and confirm ambiguous scope with the user before implementing, since the bundle is a snapshot, not a live source. Diffing it against the actual codebase (not assuming) surfaced that the snapshot predates a large amount of this app's own history: its `tokens/*.css` matched the live `frontend/src/index.css` byte-for-byte in every color/typography/effects value (confirming the app had already independently converged on the same design language), while its `styles/components.css`/`coach.css` were roughly 7–15× smaller than the live files — an earlier product snapshot, before most of the AI-differentiator and analyst-UX panels existed. The bundle also described the product as strictly three screens with a coach that "never grades, only reflects," a framing the current, much larger persona-ranking/scanner/backtest feature set no longer matches, and included a `BrandLogo` sourced from an unrelated "myDevTime" repository this codebase has no connection to.

**Decision:**

- **Diff before applying, always.** Before touching any file, every token file and the two component stylesheets were compared line-for-line against their live counterparts. This is what surfaced that the token layer needed zero changes (already adopted) and that overwriting `components.css`/`coach.css` wholesale would have deleted ~1,500 lines of already-shipped, feature-specific styling the bundle never saw.
- **Adopt only the genuinely new, safely-scoped piece: the five `core/` primitives.** `Button`, `Field`, `Checkbox`, `Segmented`, `Switch` were real additions — the app used their underlying CSS classes (`.btn-primary`, `.field`, `.check`, `.seg`/`.tf-select`/`.mini-seg`, `.switch`) directly as `className` strings scattered across ~9 files, with no shared component. Ported as typed `.tsx` (the bundle ships `.jsx` + a separate `.d.ts`, this codebase is TypeScript-only) into `frontend/src/components/core/`, then adopted at every real call site discovered by grepping for those exact class names — not a partial or token gesture.
- **Two small, deliberate deviations from the bundle's literal spec, both preserving or improving existing behavior rather than regressing it**: `Segmented`'s wrapper role is `tablist` in `thumb` mode (matching the app's own pre-existing, more semantically-correct Login/Register switch markup) instead of the bundle's uniform `role="group"` for both modes; and `Switch` gained a `disabled` prop the bundle's own `.d.ts` didn't declare, needed for the pre-existing "Essential cookies (always on)" toggle. Both are additive — no case the bundle's API covered was dropped.
- **Three items were explicitly deferred, not silently skipped, each because it requires the user's own decision, not an engineering judgment call**: the bundle's new "Panel v2" bounded-height workspace layout (`AnnotationChips`/`SessionRail`) — a real UX restructuring of the right-hand column that the bundle designed before the current app grew a dozen more panels (scanner, personas, sentiment, backtest, depot, track record) it never accounted for; the `BrandLogo`/myDevTime asset — a brand identity decision, not a technical one, and sourced from an unrelated project; and the "coach reflects, never grades" content-tone guideline, which would mean rewriting existing copy across features (persona ranking) that structurally contradict it.
- **Content/copy was not touched anywhere** — this pass is visual-component plumbing only; no user-facing string changed.

**Consequences:**

| | |
|---|---|
| (+) | Zero regression risk from the token layer — confirmed already-current rather than blindly re-applied, which would have been a no-op at best and silently reverted local drift at worst |
| (+) | Zero risk of deleting ~1,500 lines of shipped, feature-specific CSS the bundle's stale snapshot never saw — avoided by diffing line counts before considering any overwrite |
| (+) | Five new, reusable, typed primitives now exist and are adopted everywhere their underlying classes were already used by hand — real deduplication, not just new unused files |
| (+) | Nothing was adopted on assumption: the Panel v2 layout, brand logo, and content-tone guideline are each individually named as deferred, with the specific reason each needs the user's own call rather than mine |
| (-) | The five primitives are adopted only at their *discovered* call sites (~9 files) — any future button/field/switch/segmented-toggle markup elsewhere in the app was not swept, since a codebase-wide UI audit was outside this bundle-reconciliation pass's scope |
| (-) | Two ARIA details (`tablist` role, `Switch.disabled`) deviate from the bundle's literal `.d.ts` spec — a deliberate, documented choice to preserve the app's own pre-existing (and arguably more correct) accessibility semantics, not an oversight |
